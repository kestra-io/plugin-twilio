...
/**
 * Hits the real Twilio Messages API and sends an actual SMS.
 * Enabled only when all TWILIO_* credentials are present (system property or environment
 * variable), so CI and local unit runs skip it. On a trial account the recipient must be a
 * verified number.
 */
@KestraTest
...
class SendIT {
    ...
    @Test
    void sendRealSms() throws Exception {
